...
static void pb_append(PrintBuf *pb, int rank, int tid, const char *path) {
    int n = snprintf(pb->buf + pb->len, (size_t)(BATCH_CAP - pb->len), "[MPI %d OMP %d] %s\n", rank, tid, path);
    if (n <= 0) return;
    pb->len += n;
...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        if (rank == 0)
            fprintf(stderr, "Usage: %s <root> <pattern>\n", argv[0]);
        MPI_Finalize();
        return 1;
    }
    ...
    if (nwork > 0) {
        ...
        MPI_Scatterv(work, sendcounts, displs, MPI_BYTE, 
                    my_work, my_bytes, MPI_BYTE, 
                    0, MPI_COMM_WORLD);

        if (rank == 0) { free(sendcounts); free(displs); }
    }

    if (rank == 0 && work) free(work);
    Ctx ctx = { .pat = pattern, .rank = rank };
    for (int i = 0; i < my_count; i++)
        search_root(my_work[i], &ctx);
    if (my_work) free(my_work);
    MPI_Barrier(MPI_COMM_WORLD);

    if (rank == 0)
        printf("[done] search for \"%s\" in \"%s\" complete across %d ranks.\n", pattern, root, size);
    MPI_Finalize();
